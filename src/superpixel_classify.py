...
def classify_by_superpixel(iter_nums=50, class_num=20000, tag=1, dele=.030, theta=0.85, err_del=0.025):
    ...
    # print class_tag
    # ======================== 保存最终的分类结果文件 ==============================
    classified_file = open('./tmp_file/%d_class_with_regression.pickle' % tag, 'wb')
    cPickle.dump((classified_feature, classified_patch, classified_error, class_tag), classified_file, 1)

    # ======================== 保存结果文件对应的回归模型 ============================
    regression_file = open('./tmp_file/%d_regression_ridge.pickle' % tag, 'wb')
